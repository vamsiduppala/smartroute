...
- **Docker image build verification** — still can't actually build/run the image: the daemon isn't running (Docker Desktop isn't even started) in this environment, and booting it here is slow/uncertain (tried once, gave up after ~70s). What *is* now verified without needing the daemon: ran `hadolint` (the standard Dockerfile linter, downloaded as a standalone binary since no daemon means no `docker run hadolint/hadolint`) against `Dockerfile` — zero warnings, clean exit. So the Dockerfile passes static best-practices linting; it has still never actually been built into a real image. Revisit when Docker Desktop is up: `docker build -t smartroute:0.1.0 .` then a smoke-test container run.
