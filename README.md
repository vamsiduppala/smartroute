...
![Tests](https://img.shields.io/badge/tests-106%20green-success)
...
- **The cheapest call is the one you never make.** An exact-match response cache (TTL + size-bounded) serves a repeated prompt for **$0** — skipping classification, budget booking, and the model call — and reports the amount saved right on the response (`budgetDecision: CACHED`). On-thesis: caching is the most direct cost lever after routing.
...
- **106 tests; the full suite runs in CI (`mvn package`) on every push.** Unit tests, `@WebMvcTest` per-controller web slices, and end-to-end flows (allow / blocked / downgrade) through the real embedded server — plus a credit-free routing simulation on real published pricing.
...
flowchart LR
    ...
    GR -->|flagged| X
    GR -->|clean| CA[Response cache: exact match]
    CA -->|hit → $0| RESP[Return cached result]
    CA -->|miss| BG[Governance: budget check + reserve]
    BG -->|over cap| X
...
# Send the SAME prompt twice — the second is served from cache for $0:
curl -s localhost:8080/gateway/route -H 'content-type: application/json' \
     -d '{"tenant":"acme","prompt":"What is the capital of France?"}' >/dev/null   # 1st: routes
curl -s localhost:8080/gateway/route -H 'content-type: application/json' \
     -d '{"tenant":"acme","prompt":"What is the capital of France?"}'              # 2nd: cache hit
# → {"allowed":true,"budgetDecision":"CACHED","notes":["cache-hit: saved $0.00020"], ...}   no model call
