...
80 tests across the modules, all green (CI on every push) — unit tests (including a real concurrency stress test for the budget reservation logic), `@WebMvcTest` web-layer slices per controller, and end-to-end tests (ALLOW, blocked, DOWNGRADE) through the real embedded server. See `docs/*-NOTES.md` for per-module design.
