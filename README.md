...
![Tests](https://img.shields.io/badge/tests-105%20green-success)
...
- **105 tests; the full suite runs in CI (`mvn package`) on every push.** Unit tests, `@WebMvcTest` per-controller web slices, and end-to-end flows (allow / blocked / downgrade) through the real embedded server — plus a credit-free routing simulation on real published pricing.
