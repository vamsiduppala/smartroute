...
- **98 tests, green on every push.** Unit tests, `@WebMvcTest` per-controller web slices, and end-to-end flows (allow / blocked / downgrade) through the real embedded server — plus a credit-free routing simulation on real published pricing.
